...
我要做一个把自己创意想法发自媒体的系统（电脑端和手机端，浏览器访问）。刚开始是一个想法，我会用简短的话描述。然后让ai给我生成自媒体文章，包括插图。我会在手机端查看，像看新闻客户端一样。我会在每篇文章下进行评论指出文章生成的有哪些问题或者图片生成问题等。然后ai自动看到问题后自动修改。我的待阅里面会有新生成的未审核的文章。直到我审核通过该文章就定稿了。要求是免费的AI。要求资源没有国内网络限制问题。ai会自动找出有评论未修改的，或者新创意未生成文章的。我和AI都有一个我的待办页面。什么设备都不开。ai自动检测待办并工作.

现在是用界面发布文章，下一步AI自动检测到待办任务自动发布和修改文章
图片不要存成base64。直接在数据库中存成文件。最后给我一键导出所有数据附件和程序的功能
